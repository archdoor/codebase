...
typedef enum
{
    EPTIMER_ONE_SHOT,   // 单次定时器
    EPTIMER_PERSIST,    // 永久定时器
}EPTIMER_OPT;
...
typedef struct
{
    ...
    duple_list *timer_list[EPTIMER_WHEEL_SLOT_NUM];   // 时间轮各个时间槽上定时器列表
    int epfd;   // epoll句柄
...
#define EPTIMER_BASE_SIZE sizeof(eptimer_base)
#define EPTIMER_SIZE sizeof(eptimer)
